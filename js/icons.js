/**
 * Sistema de Iconos - Reemplaza emojis por iconos SVG de Lucide
 * Guarda este archivo como: js/icons.js
 */

// Mapa de iconos de Lucide (nombres en kebab-case a SVG)
const LUCIDE_ICONS = {
  'bot': '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="10" x="3" y="11" rx="2"/><circle cx="12" cy="5" r="2"/><path d="M12 7v4"/><line x1="8" x2="8" y1="16" y2="16"/><line x1="16" x2="16" y1="16" y2="16"/></svg>',
 ...
 * @param {string} iconName - Nombre del icono en formato kebab-case
 * @param {number} size - Tamaño del icono (default: 24)
 * @param {string} className - Clases CSS adicionales
 * @returns {string} HTML del icono SVG
 */
function getIcon(iconName, size = 24, className = '') {
  const svg = LUCIDE_ICONS[iconName] || LUCIDE_ICONS['bot'];
  ...
  // Reemplaza width, height, añade clase y COLOR AZUL
  return svg
    .replace(/width="24"/, `width="${size}"`)
    .replace(/height="24"/, `height="${size}"`)
    .replace(/stroke="currentColor"/, `stroke="#3b82f6"`)
    .replace(/<svg/, `<svg class="emoji-icon ${className}" style="display: inline-block; vertical-align: middle;"`);
} {string} className - Clases CSS adicionales
 * @returns {string} HTML del icono SVG
 */
function getIcon(iconName, size = 24, className = '') {
  const svg = LUCIDE_ICONS[iconName] || LUCIDE_ICONS['help-circle'];
  ...
  // Reemplaza width, height y añade clase
  return svg
    .replace(/width="24"/, `width="${size}"`)
    .replace(/height="24"/, `height="${size}"`)
...
// Agregar estilos CSS automáticamente para que los iconos se vean perfectos
if (!document.getElementById('emoji-icon-styles')) {
  const style = document.createElement('style');
  style.id = 'emoji-icon-styles';
  style.textContent = `
    .emoji-icon {
    ...
    .roadmap-icon .emoji-icon {
      width: 32px !important;
      height: 32px !important;
    }
    
    .badge-item .emoji-icon {
      width: 16px !important;
      height: 16px !important;
      margin-right: 4px;
    }
